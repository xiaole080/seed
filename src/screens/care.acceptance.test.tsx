...
describe('CareScreen 追加チェック', () => {
  ...
  it('paused のカードは最下部に並ぶ', () => {
    ...
    localStorage.setItem(
      ROUTINES_KEY,
      JSON.stringify(
        list.map((x) => (x.id === paused.id ? { ...x, paused: true } : x))
      )
    );
    renderCare();
    ...
    expect(
      active.compareDocumentPosition(sleepy) & Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy();
  });
  ...
  it('同じ routine の達成を 2 回タップすると取り消される', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '達成を取り消す' }));
  ...
  it('OneOff の自由記述: 絵文字・スクリプト風文字列でも壊れない', async () => {
    ...
    await user.type(screen.getByPlaceholderText('例: 朝、窓を開ける'), evil);
    await user.click(screen.getByRole('button', { name: '今日だけ' }));
    const tasks = loadOneOffTasks();
    expect(tasks).toHaveLength(1);
...
describe('CareScreen 文言走査', () => {
  it('フッターに「達成できなくても、大丈夫。」が残っている', () => {
    renderCare();
    expect(screen.getByText(/達成できなくても、大丈夫。/)).toBeInTheDocument();
  });
