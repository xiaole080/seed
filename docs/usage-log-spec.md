...
- 設定画面（ProfileScreen）への「利用ログ」カード追加
  - イベント一覧表示（テキストのみ）
  - 簡単な集計: 直近 7 日の「記録回数 / 起動回数 / 閲覧回数 / 平均記録時間（秒）」
...
- 内容:
  - `summarizeUsage(events, now)` — 直近 7 日（now 含む 7 日間）の
...
- Given 直近 7 日に record_save 3 件・app_open 5 件・history_open 2 件がある When 利用ログカードを開く Then 記録回数 3 / 起動回数 5 / 閲覧回数 2 が表示される
...
- Given 8 日以上前のイベントのみがある When カードを開く Then 直近 7 日の各集計は 0 と表示される（エラーにならない）
